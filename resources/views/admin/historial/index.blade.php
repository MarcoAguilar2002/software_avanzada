...
@section('title', 'Lista de Historial')
        ...
        <table id="example1" class="table table-hover ">
            ...
            <tbody class="text-center">
                ...
                @foreach ($historiales as $historiale)
                    ...
                    @if ($historiale->doctor_id == Auth::user()->id)
                        <tr>
                            <td>{{ $contador++ }}</td>
                            <td>{{ $historiale->paciente->apellidos.' '.$historiale->paciente->nombres }}</td>
                            <td>{{ $historiale->fechaVisita }}</td>
                            <td>{!! $historiale->detalle !!}</td>
                            <td>
                                <a href="{{ route('admin.historials.show', $historiale->id) }}" class="btn btn-primary"
                                    style="background-color: blue;">
                                    <i class="bi bi-eye-fill"></i>
                                </a>
                                <a href="{{ route('admin.historials.pdf', $historiale->id) }}" class="btn btn-warning"
                                    >
                                    <i class="bi bi-printer-fill"></i>
                                </a>
                                <a href="{{ route('admin.historials.edit', $historiale->id) }}" class="btn btn-success"
                                    style="background-color: green;">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.historials.destroy', $historiale->id) }}" method="POST"
